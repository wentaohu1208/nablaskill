...
class SequentialSkillTrainer:
    ...
    def optimize(
        ...
        if self.show_train_pbar and not self.show_train_logs:
            pbar = tqdm.tqdm(total=num_positions, desc="Sequential DTO")
        else:
            pbar = None
        ...
        while not states.is_done:
            ...
            if pbar is not None:
                pbar.update(n_commit)

            self._log(
                "Committed %d token(s) at pos %d/%d: '%s'",
                n_commit, states.num_past, num_positions,
        ...
        logger.info(
            "Rejection sampling: %d accepted, %d rejected (out of %d changed)",
